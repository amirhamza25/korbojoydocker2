...
const AgentOrderSuccess = (props) => {
  ...
  useEffect(() => {
    const sjdklf = axios.get(process.env.API_URL + "/AdminAgent/v1/getPurchaseProductToCompanySuccess").then((res) => {
  ...
  //     });
  // };
  // const cancelOrders = useRecoilValue(storeAgentSuccessListOfProduct);
  return (
    <div>
      <div className="row">
...
export async function getServerSideProps(context) {
  const { data } = await axios.get(process.env.API_URL + "/AdminAgent/v1/getPurchaseProductToCompanySuccess");
  // console.log(data)
  if (!data) {
    return {
      notFound: true,
